public class Solution {
    ...
    public static int findAvgEnginePriceByType(Engine[] eng, String type) {
        int count = 0;
        double sum = 0;
        for (Engine e : eng) {
    ...
    public static Engine[] searchEngineByName1(Engine[] eng, String name) {
    ...
    // Sort by ID if needed
        result.sort(Comparator.comparingInt(Engine::getId));
        
        return result.toArray(new Engine[0]);
        return result.isEmpty() ? null : result.toArray(new Engine[0]);
    }
